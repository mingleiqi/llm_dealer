...
#### 1. 查询目的
对指定股票进行全面分析，预测其未来1-3个月的走势，为投资决策提供参考。

#### 2. 数据收集

1. 股票基本信息：
   - 使用 `search_stock_code(stock_name)` 获取股票代码 `stock_code`
   - 使用 `get_stock_info(stock_code)` 获取股票基本信息
   - 使用 `get_main_business_description(stock_code)` 获取主营业务描述

2. 行情数据：
   - 使用 `get_full_realtime_data()[stock_code]` 获取实时行情数据
   - 使用 `summarize_historical_data(stock_code, days=60)` 获取近60个交易日的历史数据摘要
   - 使用 `get_stock_a_indicators(stock_code)` 获取A股个股指标

3. 财务数据：
   - 使用 `get_financial_analysis_summary(stock_code)` 获取财务分析摘要
   - 使用 `get_key_financial_indicators(stock_code)` 获取关键财务指标

4. 市场情绪和分析：
   - 使用 `get_stock_comments_summary()` 获取千股千评数据
   - 使用 `get_baidu_analysis_summary(stock_code)` 获取百度个股分析摘要

5. 新闻和公告：
   - 使用 `get_stock_news(stock_code, num=10)` 获取最新的10条相关新闻
   - 使用 `get_company_announcements(stock_code, num=5)` 获取最新的5条公司公告

6. 行业数据：
   - 使用 `select_stocks_by_industry_board_query(f"查询{stock_name}所属行业的平均市盈率、平均市净率")` 获取行业平均估值数据
   - 使用 `get_industry_lifecycle_stage(industry_code)` 获取行业生命周期阶段（假设此函数存在）

7. 机构评级和资金流向：
   - 使用 `get_investment_ratings()` 获取最新投资评级
   - 使用 `get_individual_stock_fund_flow_rank()` 获取资金流排名

8. 宏观经济数据：
   - 使用 `get_macro_economic_indicators()` 获取主要宏观经济指标
   - 使用 `get_global_economic_indicators()` 获取全球经济指标（假设此函数存在）

9. 公司治理和ESG数据：
   - 使用 `get_management_team_info(stock_code)` 获取管理团队信息（假设此函数存在）
   - 使用 `get_esg_score(stock_code)` 获取ESG评分（假设此函数存在）

10. 竞争对手数据：
    - 使用 `get_main_competitors(stock_code)` 获取主要竞争对手信息（假设此函数存在）

#### 3. 数据分析和处理步骤

1. 整合所有收集到的数据，形成综合数据集。

2. 进行定量分析：
   - 计算关键财务比率（如ROE、资产负债率等）
   - 计算技术指标（如MACD、RSI等）
   - 与行业平均水平和主要竞争对手进行对比

3. 使用LLM进行初步分析，生成分析报告。提示词如下：

   作为一位资深的股票分析师，请根据以下信息对 [股票代码] [股票名称] 进行全面分析并预测其未来1-3个月走势：

   [插入所有收集到的数据]

   请提供以下分析：

   1. 执行摘要（200字以内）：概括关键发现和预测结果。
   2. 基本面分析（300字以内）：评估公司的财务状况、盈利能力、成长性和行业地位。
   3. 技术面分析（300字以内）：分析近期股价走势、成交量变化、主要技术指标信号。
   4. 市场情绪分析（200字以内）：解读市场对该股票的整体看法和情绪。
   5. 行业分析（250字以内）：评估行业状况、生命周期阶段，以及公司在行业中的竞争地位。
   6. 宏观经济分析（200字以内）：分析当前国内外宏观经济环境对该股票的潜在影响。
   7. 公司治理和ESG分析（200字以内）：评估公司治理结构、管理团队质量和ESG表现。
   8. 新闻和公告影响分析（200字以内）：分析近期新闻和公告对股价的潜在影响。
   9. 机构评级解读（150字以内）：解读机构对该股票的评级和目标价。
   10. 资金流向分析（150字以内）：分析近期资金流入/流出情况及其对股价的影响。
   11. 短期预测（1个月内）（200字以内）：基于以上分析，预测股票短期可能的走势。
   12. 中期预测（1-3个月）（200字以内）：基于以上分析，预测股票中期可能的走势。
   13. 风险评估（200字以内）：使用风险矩阵评估和呈现各种潜在风险。
   14. 情景分析（250字以内）：提供最佳、最差和基准三种情景下的预测。
   15. 预测置信度评估（100字以内）：基于数据质量和市场不确定性，评估预测的置信度。
   16. 投资建议（200字以内）：根据分析结果，提供短期操作建议和中期投资策略。

   请确保分析客观、全面，并提供有见地的洞察。您的分析将帮助投资者理解该股票的近期表现并为投资决策提供参考。
   请以JSON格式返回您的分析结果，包含上述16个字段。

4. 基于LLM的初步分析结果，再次调用LLM生成最终的个股行情预测报告。

5. 使用机器学习模型（如果可用）生成预测结果，并与LLM分析结果进行对比。

#### 4. 预期输出格式和内容

生成一份结构化的个股行情预测报告，包含以下内容：
...
- 保持分析的客观性和全面性
- 结合定量分析和定性分析
- 考虑宏观经济环境、行业趋势和公司特定因素对个股的综合影响
- 根据股票类型（如成长股、价值股、周期股）调整分析重点
- 在提供投资建议时，强调风险管理的重要性
- 使用清晰、易懂的语言，避免过于专业的术语
- 提醒投资者，预测结果仅供参考，不构成投资建议
- 说明预测模型的局限性，如无法预见突发事件的影响
- 建议投资者在做出投资决策前，进行进一步的研究和咨询专业意见
- 考虑提供交互式数据可视化选项，增强报告的可读性和理解性