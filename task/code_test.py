...
def runner():
    ...
    # Step 1: 获取热门股票和市场信息
    hot_stocks = stock_data_provider.get_baidu_hotrank(num=300)
    market_overview = stock_data_provider.stock_market_desc()
    market_news = stock_data_provider.get_market_news_300()
    market_news_summary = stock_data_provider.summarizer_news(market_news, "总结行业热点，市场机会，市场风险点，以及可能影响短期走势的关键因素")
    index_trend = stock_data_provider.summarize_historical_index_data(['000001', '399001', '399006'])
    ...
    for stock in hot_stocks:
        ...
        stock_news = stock_data_provider.get_one_stock_news(stock, num=3)
    ...
    for stock_code, stock_data in stock_analysis.items():
        prompt = f"""
        请对股票{stock_data['name']}({stock_code})进行短线潜力分析，特别关注市场热点对该股票的影响，考虑以下因素：
        ...
        - 市场新闻摘要中的热点：{market_news_summary}
        ...
        5. 短期催化剂或利好因素
        6. 潜在风险因素
        7. 注意个股新闻的时间，可能因为近期没有重要新闻，读取的是过往新闻

        返回JSON格式，包含：
        - 'market_hot_spot': 从新闻归纳的市场热点
        - 'code': 股票代码
        - 'name': 股票名称
        - 'score': 短线潜力评分（0-100的整数）
        - 'attention': 市场关注度（0-100的整数）
        - 'hotspot_relevance': 热点相关性评分（0-100的整数）
    ...
    # Step 4: 筛选和排序
    sorted_stocks = sorted(stock_evaluations.values(), key=lambda x: (x['score'], x['hotspot_relevance'], x['attention']), reverse=True)
    recommended_stocks = sorted_stocks[:10]
    for stock in recommended_stocks:
        stock['composite_score'] = stock['score'] * 0.5 + stock['attention'] * 0.3 + stock['hotspot_relevance'] * 0.2
