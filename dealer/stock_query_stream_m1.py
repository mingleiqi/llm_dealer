...
class StockQueryStream:
    ...
    def _generate_plan(self, query: str) -> Generator[Dict[str, Any], None, None]:
        ...
        对于涉及 LLM 分析的步骤，请在伪代码中详细说明：
        1. 构建详细的提示词，包含所有必要的数据和上下文信息。
        2. 提示词中明确指定 LLM 输出应为 JSON 格式。
        3. 提示词中包含了"模板"中所要求的内容。
        4. 使用 llm_client.one_chat(prompt) 来调用 LLM 进行分析（不使用 SSE）。
    ...
    def _generate_step_code(self, step: dict, query: str) -> tuple:
        ...
        根据以下函数文档，生成用于理解数据结构的Python代码：

        {functions_docs}

        请生成代码来调用这些函数并创建数据结构摘要。代码应该：
        1. 调用相关函数获取数据
        2. 分析获取的数据结构
        3. 创建一个描述数据结构的摘要
        4. 使用 code_tools.add("data_structure_summary", summary) 存储摘要

        请只提供 Python 代码，不需要其他解释。
        """
        understanding_code = self._extract_code(self.llm_client.one_chat(understanding_prompt))
        
        # 执行数据结构理解代码
        understanding_result = self.code_runner.run(understanding_code)
        if understanding_result['error']:
            logger.error(f"数据结构理解代码执行失败：{understanding_result['error']}")
            data_structure_summary = "无法获取数据结构摘要"
        else:
            data_structure_summary = code_tools.get("data_structure_summary", "无法获取数据结构摘要")

        # 生成主要代码
        main_prompt = f"""
        根据以下步骤信息、函数文档和数据结构摘要，生成可执行的Python代码：
        ...
        对于涉及 LLM 分析的步骤，即便伪代码没有提及，也需要确保：
        1. 构建详细的提示词，包含所有必要的数据和上下文信息。
        ...
        请只提供 Python 代码，不需要其他解释。
        """
        main_code = self._extract_code(self.llm_client.one_chat(main_prompt))
        logger.info("步骤代码生成完成")
        return main_code, main_prompt
